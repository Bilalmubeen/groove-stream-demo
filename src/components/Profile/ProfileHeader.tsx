  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-6">
        <Avatar className={cn("border-2 border-primary/20", isMobile ? "w-20 h-20" : "w-24 h-24 sm:w-32 sm:h-32")}>
        ...
        <div className="flex-1 space-y-3 md:space-y-4 w-full">
          ...
          <div className={cn("flex items-center gap-4 md:gap-6", isMobile ? "text-xs" : "text-sm")}>
            <button onClick={() => navigate(`/u/${profile?.username}/followers`)} className="hover:text-primary transition-all">
          ...
          {profile?.bio && (
            <p className={cn("text-foreground max-w-md line-clamp-3", isMobile ? "text-xs" : "text-sm")}>
              {profile.bio}
            </p>
          )}
          ...
          <div className={cn("flex flex-wrap items-center gap-2 md:gap-3 pt-2")}>
            {isOwnProfile ? (
              <>
                <Button variant="default" onClick={onUploadClick} size={isMobile ? "sm" : "default"} className="flex-1 sm:flex-initial">
                  <Upload className={cn(isMobile ? "w-3 h-3" : "w-4 h-4", !isMobile && "mr-2")} />
                  {!isMobile && "Upload Snippet"}
                </Button>
                <Button variant="outline" onClick={onEditClick} size={isMobile ? "sm" : "default"} className={isMobile ? "flex-1" : ""}>
                  {isMobile ? "Edit" : "Edit Profile"}
                </Button>
                {isArtist && (
                  <Button variant="outline" onClick={() => navigate('/analytics')} size={isMobile ? "sm" : "default"}>
                    <BarChart3 className={isMobile ? "w-3 h-3" : "w-4 h-4"} />
                  </Button>
                )}
                <Button variant="outline" onClick={() => navigate('/settings')} size={isMobile ? "sm" : "default"}>
                  <Settings className={isMobile ? "w-3 h-3" : "w-4 h-4"} />
                </Button>
              </>
            ) : (
              <>
                <Button variant={isFollowing ? 'outline' : 'default'} onClick={handleFollow} size={isMobile ? "sm" : "default"} className="flex-1 sm:flex-initial">
                  {isFollowing ? (
                    <>
                ...
                <Button variant="outline" onClick={() => navigate('/messages')} size={isMobile ? "sm" : "default"} className="flex-1 sm:flex-initial">
